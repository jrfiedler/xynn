"""
PyTorch module for the Attn-TCN model
...
class AttnTCN(BaseNN):
    ...
    def __init__(
        ...
        self.tcn = TemporalConvNet(
            ...
            num_inputs=(attn_embedding_size * attn_num_heads),
        ...
        if self.tcn_output == "all":
            mlp_input_size = tcn_hidden_sizes[-1] * embed_info.num_fields
        else:
            mlp_input_size = tcn_hidden_sizes[-1]
    ...
    def forward(self, X_num: Tensor, X_cat: Tensor) -> Tensor:
        ...
        out_main = self.attn_interact(out_main)
        out_main = self.tcn(out_main.movedim(2, 1))
        if self.tcn_output == "all":
            out_main = out_main.flatten(start_dim=1)
        elif self.tcn_output == "temporal":
            out_main = out_main[:, :, -1]
        elif self.tcn_output == "non-temporal":
            out_main = out_main[:, :, out_main.shape[-1] // 2]
        out_main = self.main_mlp(out_main)

        out_side = self.side_mlp(embedded.flatten(start_dim=1))
